...
const AdminForm = ({ categories, skins, bodyparts, packs, validator, dataToUpdate }: AdminProductsAddProps) => {
...
- data stock la saisie du formulaire*/}
  ...
  const submitForm = async (data: Partial<Product>) => {
    ...
    formData.set("description", normalizedData.description as unknown as string);
    formData.set("price", normalizedData.price as unknown as string);
    formData.set("category_id", normalizedData.category_id as unknown as string);
    formData.set("skin_ids", normalizedData.skin_ids as unknown as string);
    formData.set("body_part_ids", normalizedData.body_part_ids as unknown as string);
...
*/}
        ...
        <div>
          ...
          {<input type="text" id={priceId} placeholder="Prix" {...register('price', { required: "Le prix est obligatoire", min: { value: 1, message: "Un prix doit comporter au minimum 1 chiffre" }, max: { value: 99.99, message: "Un prix peut comporter au maximum 4 caracteres dont 2 decimales" } })} />}
